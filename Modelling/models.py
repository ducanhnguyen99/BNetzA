...
def xgboost_regression(df_train, df_test, target, model_name, outcome_transformation="None", random_state=42, feature_selection_threshold=0.1):
    """Performs XGBoost regression with cross-validation, feature importance-based selection, and early stopping."""
    # Split data into features and target
    X_train = df_train.drop(columns=[target])
    y_train = df_train[target]
    ...
    importance_df = pd.DataFrame(importance_dict).sort_values(by='Importance', ascending=False).reset_index(drop=True)
